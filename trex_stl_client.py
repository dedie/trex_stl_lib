...
class STLClient(object):
    ...
    def reacquire_line (self, line):
        ...
        my_unowned_ports = list_difference([k for k, v in list(self.ports.items()) if v.get_owner() == self.username], self.get_acquired_ports())
    ...
    def set_port_attr_line (self, line):
        ...
        if not list([opt for opt in opts._get_kwargs() if opt[0] not in ('all_ports', 'ports') and opt[1] is not None]):
    ...
    def show_profile_line (self, line):
        ...
        if profile_type == 'python':
            ...
            self.logger.log('Tunables:         {:^12}'.format(str(['{0} = {1}'.format(k ,v) for k, v in list(info['tunables'].items())])))
