...
class CTRexInfoGenerator(object):
    ...
    def _generate_streams_stats (self):
        ...
        sstats_new_data = OrderedDict([(disp_str[k], v) if k in disp_str else (k, v) for k, v in list(sstats_data.items())])
    ...
    def _generate_port_stats(self, port_id_list):
        ...
        disp_data = OrderedDict([(disp_str[k], v) if k in disp_str else (k, v) for k, v in list(per_field_stats.items())])
...
class CRxStats(CTRexStats):
    ...
    def process_snapshot (self, current, prev):
        ...
        ref_pg_ids = list(filter(is_intable, list(self.reference_stats.keys())))
