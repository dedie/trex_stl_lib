...
class DomainNameListField(StrLenField):
    ...
    def i2m(self, pkt, x):
        ...
        tmp = [list(map((lambda z: chr(len(z)).encode('ascii')+z), y.split(b'.'))) for y in x]
        return b"".join([conditionalTrailingDot(b"".join(x)) for x in tmp])
...
class DomainNameField(StrLenField):
    ...
    def i2m(self, pkt, x):
        if not x:
            return b""
        tmp = b"".join([chr(len(z)).encode('ascii')+z for z in x.split(b'.')])
        return tmp
   ...
   If you have a need for others, just ask ... or provide a patch."""
    ...
    def parse_options(self, dns="2001:500::1035", domain="localdomain, local",
        ...
        if duid is not None:
            ...
            mac = ":".join(["%.02x" % x for x in rawmac])
