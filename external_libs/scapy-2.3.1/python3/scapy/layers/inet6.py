...
def names2dnsrepr(x):
    ...
    for n in x:
        ...
        if n.count(b'.') == 0: # single-component gets one more
            termin += bytes([0]) 
        n = b"".join([chr(len(y)).encode('ascii')+y for y in n.split(b".")]) + termin
        res.append(n)
    return b"".join(res)
...
class NIReplyDataField(StrField):
    ...
    def addfield(self, pkt, s, val):
        ...
        if t == 2:
            ...
            return s + b"".join([struct.pack("!I", a[0])+inet_pton(socket.AF_INET6, a[1]) for a in tmp])
        elif t == 4:
            #return s + "".join(map(lambda(x,y): struct.pack("!I", x)+inet_pton(socket.AF_INET, y), tmp))
            return s + b"".join([struct.pack("!I", a[0])+inet_pton(socket.AF_INET, a[1]) for a in tmp])
        else:
            return s + tmp
    ...
    def i2repr(self, pkt, x):
        ...
        if type(x) is tuple and len(x) == 2:
            ...
            if t == 2: # DNS names
                ...
                return "[ %s ]" %(", ".join(["(%d, %s)" % a for a in val]))
            return repr(val)
