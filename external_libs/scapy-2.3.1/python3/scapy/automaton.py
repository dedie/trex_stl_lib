...
class Automaton_metaclass(type):
    def __new__(cls, name, bases, dct):
        ...
        decorated = [v for v in list(members.values())
        ...
        for v in list(cls.timeout.values()):
            #v.sort(lambda (t1,f1),(t2,f2): cmp(t1,t2))
            v.sort(key = lambda x: x[0])
            v.append((None, None))
        for v in itertools.chain(list(cls.conditions.values()),
                                 list(cls.recv_conditions.values()),
                                 list(cls.ioevents.values())):
            v.sort(key = lambda x: x.atmt_prio)
        for condname,actlst in list(cls.actions.items()):
